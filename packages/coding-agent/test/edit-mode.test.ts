...
describe("resolveEditMode", () => {
	...
	test("falls back from hashline to replace for MiMo and DeepSeek models", () => {
		delete Bun.env.PI_EDIT_VARIANT;

		expect(resolveEditMode(createSession({ activeModel: "xiaomi/MiMo-V2.5-Pro" }))).toBe("replace");
		expect(resolveEditMode(createSession({ activeModel: "deepseek/deepseek-v4-flash" }))).toBe("replace");
	});
